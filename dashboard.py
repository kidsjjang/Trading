...
from hermes_trading_agents import DEFAULT_MODEL as DEFAULT_HERMES_MODEL
from hermes_trading_agents import HermesTradingError, run_hermes_review_payload
...
def get_hermes_review(model: str, payload_json: str):
    return run_hermes_review_payload(model=model, payload=json.loads(payload_json))
...
def render_mapping_agent_result(role: str, result: dict) -> None:
    ...
    st.markdown(f"#### {role}")
    st.metric("Agent View", str(result.get("decision", "NEUTRAL")).upper(), f"{confidence_value:.0%} confidence")
    st.write(result.get("summary") or "No summary returned.")
...
with st.sidebar:
    ...
    agent_backend = "Direct Ollama"
    if enable_local_llm:
        agent_backend = st.radio(
            "Agent runtime",
            ["Direct Ollama", "Hermes Agent Lite"],
            index=0,
            help="Hermes Agent Lite uses the local Hermes Python runtime and is slower on CPU.",
        )
        try:
            available_models = list_ollama_models()
            if available_models:
                preferred_model = DEFAULT_HERMES_MODEL if agent_backend == "Hermes Agent Lite" else "qwen3:4b-instruct"
        ...
        if agent_backend == "Hermes Agent Lite":
            st.caption("Hermes Agent Lite is experimental and can take about 1-3 minutes on this CPU-only PC.")
...
try:
    ...
    if enable_local_llm:
        overlay_title = "Hermes Agent Lite Overlay" if agent_backend == "Hermes Agent Lite" else "Local LLM Agent Overlay"
        st.subheader(overlay_title)
        st.markdown(
            "<div class='llm-note'>Local agent output is a second opinion generated on this computer. "
        ...
        if agent_backend == "Hermes Agent Lite":
            try:
                st.info("Hermes Agent Lite is slow on this CPU-only PC, so it runs only when you click the button.")
                if st.button("Run Hermes Agent Lite review", use_container_width=True):
                    with st.spinner(f"Running Hermes Agent Lite with {selected_llm_model}..."):
                        hermes_review = get_hermes_review(selected_llm_model, payload_json)
                    st.session_state["last_hermes_payload_json"] = payload_json
                    st.session_state["last_hermes_review"] = hermes_review

                hermes_review = None
                if st.session_state.get("last_hermes_payload_json") == payload_json:
                    hermes_review = st.session_state.get("last_hermes_review")

                if hermes_review:
                    hermes_agents = hermes_review["hermes_agents"]
                    llm_left, llm_middle, llm_right = st.columns(3)
                    with llm_left:
                        with st.container(border=True):
                            render_mapping_agent_result("Researcher", hermes_agents["researcher"])
                    with llm_middle:
                        with st.container(border=True):
                            render_mapping_agent_result("Trader", hermes_agents["trader"])
                    with llm_right:
                        with st.container(border=True):
                            render_mapping_agent_result("Risk Manager", hermes_agents["risk_manager"])
            except (HermesTradingError, KeyError) as exc:
                st.error(str(exc))
        else:
            try:
